...
<body>
						...
						<table class="table table-hover table-striped" id="list-table">
							...
							<tr>
								...
								<th><input type="checkbox" class="checkall" id="checkall"
									onclick='selectAll(this)'> <label for="checkall"><em></em>전체선택</label><th>댓글번호</th>
							...
							<c:if test="${!empty QList}">
								<c:forEach var="qlist" items="${QList}" varStatus="vs">
									...
									<tr class="cklist">
										<td><input type="checkbox" name="ck" class="checkone"
											id="checkone${vs.count}" value="${qlist.qaNo}"> <label
											for="checkone${vs.count}"><em></em></label></td>
	...
	<script>
		$(".cklist").on(
				"click",
				function() {
					var placeNo = $(this).children().eq(5).text();
					var placeNo1 = $(this).children().eq(4).text();

					console.log(placeNo);

					var placeViewURL = "${contextPath}/shop/" + placeNo + "/"
							+ placeNo1;
